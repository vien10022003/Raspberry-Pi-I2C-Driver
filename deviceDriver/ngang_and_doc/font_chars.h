#ifndef FONT_CHARS_H
#define FONT_CHARS_H

#include <linux/types.h>

const uint8_t font_8x8[][8] = {
    // A (index 0)
    {0x00, 0x18, 0x3C, 0x66, 0x7E, 0x66, 0x66, 0x00},
    // B (index 1)
    {0x00, 0x7C, 0x66, 0x7C, 0x66, 0x66, 0x7C, 0x00},
    // C (index 2)
    {0x00, 0x3C, 0x66, 0x60, 0x60, 0x66, 0x3C, 0x00},
    // D (index 3)
    {0x00, 0x7C, 0x66, 0x66, 0x66, 0x66, 0x7C, 0x00},
    // E (index 4)
    {0x00, 0x7E, 0x60, 0x7C, 0x60, 0x60, 0x7E, 0x00},
    // F (index 5)
    {0x00, 0x7E, 0x60, 0x7C, 0x60, 0x60, 0x60, 0x00},
    // G (index 6)
    {0x00, 0x3C, 0x66, 0x60, 0x6E, 0x66, 0x3C, 0x00},
    // H (index 7)
    {0x00, 0x66, 0x66, 0x7E, 0x66, 0x66, 0x66, 0x00},
    // I (index 8)
    {0x00, 0x7E, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00},
    // J (index 9)
    {0x00, 0x06, 0x06, 0x06, 0x06, 0x66, 0x3C, 0x00},
    // K (index 10)
    {0x00, 0x66, 0x6C, 0x78, 0x6C, 0x66, 0x66, 0x00},
    // L (index 11)
    {0x00, 0x60, 0x60, 0x60, 0x60, 0x60, 0x7E, 0x00},
    // M (index 12)
    {0x00, 0x63, 0x77, 0x7F, 0x6B, 0x63, 0x63, 0x00},
    // N (index 13)
    {0x00, 0x66, 0x76, 0x7E, 0x6E, 0x66, 0x66, 0x00},
    // O (index 14)
    {0x00, 0x3C, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00},
    // P (index 15)
    {0x00, 0x7C, 0x66, 0x66, 0x7C, 0x60, 0x60, 0x00},
    // Q (index 16)
    {0x00, 0x3C, 0x66, 0x66, 0x66, 0x6E, 0x3C, 0x03},
    // R (index 17)
    {0x00, 0x7C, 0x66, 0x66, 0x7C, 0x6C, 0x66, 0x00},
    // S (index 18)
    {0x00, 0x3C, 0x60, 0x3C, 0x06, 0x06, 0x3C, 0x00},
    // T (index 19)
    {0x00, 0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x00},
    // U (index 20)
    {0x00, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00},
    // V (index 21)
    {0x00, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x00},
    // W (index 22)
    {0x00, 0x63, 0x63, 0x6B, 0x7F, 0x77, 0x63, 0x00},
    // X (index 23)
    {0x00, 0x66, 0x3C, 0x18, 0x3C, 0x66, 0x66, 0x00},
    // Y (index 24)
    {0x00, 0x66, 0x66, 0x3C, 0x18, 0x18, 0x18, 0x00},
    // Z (index 25)
    {0x00, 0x7E, 0x06, 0x0C, 0x18, 0x30, 0x7E, 0x00},
    // 0 (index 26)
    {0x00, 0x3C, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00},
    // 1 (index 27)
    {0x00, 0x18, 0x38, 0x18, 0x18, 0x18, 0x7E, 0x00},
    // 2 (index 28)
    {0x00, 0x3C, 0x06, 0x0C, 0x18, 0x30, 0x7E, 0x00},
    // 3 (index 29)
    {0x00, 0x3C, 0x06, 0x1C, 0x06, 0x06, 0x3C, 0x00},
    // 4 (index 30)
    {0x00, 0x0C, 0x1C, 0x2C, 0x7E, 0x0C, 0x0C, 0x00},
    // 5 (index 31)
    {0x00, 0x7E, 0x60, 0x7C, 0x06, 0x06, 0x7C, 0x00},
    // 6 (index 32)
    {0x00, 0x3C, 0x60, 0x7C, 0x66, 0x66, 0x3C, 0x00},
    // 7 (index 33)
    {0x00, 0x7E, 0x06, 0x0C, 0x18, 0x18, 0x18, 0x00},
    // 8 (index 34)
    {0x00, 0x3C, 0x66, 0x3C, 0x66, 0x66, 0x3C, 0x00},
    // 9 (index 35)
    {0x00, 0x3C, 0x66, 0x3E, 0x06, 0x06, 0x3C, 0x00},
    // Space (index 36)
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    // : (index 37)
    {0x00, 0x00, 0x18, 0x18, 0x00, 0x18, 0x18, 0x00},
    // , (index 38)
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x30},
    // . (index 39)
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x00}};

// Hàm chuyển ký tự thành index font
int char_to_index(char c)
{
    if (c >= 'A' && c <= 'Z')
        return c - 'A';
    if (c >= 'a' && c <= 'z')
        return c - 'a'; // lowercase = uppercase
    if (c >= '0' && c <= '9')
        return c - '0' + 26;
    if (c == ' ')
        return 36;
    if (c == ',')
        return 37;
    if (c == '.')
        return 38;
    if (c == ':')
        return 39;
    return 36; // default space
}

#endif