...
/* ✅ Keyboard handler được optimize và debug */
static int keyboard_notify(struct notifier_block *nblock, unsigned long code, void *_param)
{
    struct keyboard_notifier_param *param = _param;

    printk(KERN_DEBUG "Keyboard event: code=%lu, value=%d, down=%d\n", code, param->value, param->down);
...
static int __init scroll_module_init(void)
{
    ...
    if (ret)
    {
        printk(KERN_ERR "Cannot register keyboard notifier, error code: %d\n", ret);
        destroy_workqueue(scroll_wq);
        return ret;
    }
